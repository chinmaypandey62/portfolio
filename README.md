<p align="right">
<img src="https://visitor-badge.laobi.icu/badge?page_id=chinmaypandey62" alt="Profile Visitors"/>
</p>

<!-- Professional Introduction Card -->
<div align="center">
<div style="max-width: 1000px; margin: 0 auto;">
<table>
<tr>
<td align="center" width="800">

# **Chinmay Pandey**
**Full Stack Developer & AI/ML Enthusiast**

📍 **Location:** Kanpur, India 🇮🇳  
🎓 **Education:** B.Tech CS (2026) | GPA: 8.5/10.0  
🏆 **Competitive:** 900+ LeetCode • Top 9% Globally  
⚡ **Current Focus:** AI/ML Engineering, System Design

<table>
<tr><td>

<pre>
const developer = {
  name: "Chinmay Pandey",
  role: "Full Stack Developer 🧙‍♂️",
  skills: ["Python", "JavaScript", "React", "AI/ML"],
  motto: "Coffee → Code → Solve → Repeat ☕"
};
</pre>

</td></tr>
</table>


<!-- Quick Stats & Links -->
<div align="center">

[![Portfolio](https://img.shields.io/badge/🌟_Portfolio-FF5722?style=for-the-badge&logoColor=white)](http://chinmaypandey62.github.io/portfolio)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/chinmaypandey62)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/chinmaypandey62)
[![LeetCode](https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black)](https://leetcode.com/u/chinmaypandey62)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:2201641520062@psit.ac.in)

</div>

</td>
</tr>
</table>
</div>
</div>

<br>

<!-- Currently Building Section -->
<div style="max-width: 1000px; margin: 0 auto;">

## 🎯 Currently Building

<center>
<table>
<tr>
<td>

**🗣️ VOICE** - AI Language Assistant  
*AI-Powered Language Tutor with 100% voice interaction*
- Whisper ASR integration
- Fine-tuned 110M-parameter LLMs
- <3 seconds response latency
- 12% WER improvement

</td>
<td>

**🎵 Chord** - Social Video Platform  
*Social Platform with Synchronized Experiences*
- Real-time chat with Socket.IO
- WebRTC video calling
- Synchronized video playback
- Seamless state management

</td>
</tr>
</table>
</center>
</div>
<br>

<div style="max-width: 1000px; margin: 0 auto;">

## 🛠️ Technical Arsenal

<center>
  
### Programming Languages
<p align="center">
<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
<img src="https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black" />
<img src="https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white" />
<img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white" />
<img src="https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white" />
</p>

### Frontend Development
<p align="center">
<img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB" />
<img src="https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white" />
<img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" />
<img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css&logoColor=white" />
<img src="https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white" />
</p>

### Backend & Database
<p align="center">
<img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white" />
<img src="https://img.shields.io/badge/Express.js-000000?style=for-the-badge&logo=express&logoColor=white" />
<img src="https://img.shields.io/badge/MongoDB-4EA94B?style=for-the-badge&logo=mongodb&logoColor=white" />
<img src="https://img.shields.io/badge/Firebase-039BE5?style=for-the-badge&logo=Firebase&logoColor=white" />
</p>

### AI/ML & Data Science
<p align="center">
<img src="https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white" />
<img src="https://img.shields.io/badge/scikit_learn-F7931E?style=for-the-badge&logo=scikit-learn&logoColor=white" />
<img src="https://img.shields.io/badge/Pandas-2C2D72?style=for-the-badge&logo=pandas&logoColor=white" />
<img src="https://img.shields.io/badge/NumPy-777BB4?style=for-the-badge&logo=numpy&logoColor=white" />
</p>
</center>
</div>
<br>

<div style="max-width: 1000px; margin: 0 auto;">

## 📊 GitHub Analytics & Competitive Programming

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=chinmaypandey62&show_icons=true&theme=tokyonight&hide_border=true&count_private=true" width="420" height="200" />
<img src="https://github-readme-streak-stats.herokuapp.com/?user=chinmaypandey62&theme=tokyonight&hide_border=true" width="420" height="200" />

<br><br>

<p align="center">
  <a href="https://leetcode.com/chinmaypandey62">
    <img src="https://leetcard.jacoblin.cool/chinmaypandey62?theme=catppuccinMocha&font=Stylish&ext=contest" alt="LeetCode Stats" width="400" height="280" style="margin: 10px;" />
  </a>
  <a href="https://codeforces.com/profile/chinmaypandey62">
    <img src="https://codeforces-readme-stats.vercel.app/api/card?username=chinmaypandey62&theme=github_dark&disable_animations=false&show_icons=true&force_username=true" alt="Codeforces Stats" width="400" height="280" style="margin: 10px;" />
  </a>
</p>

### 🏆 Key Achievements
🥇 **LeetCode**: 800+ problems solved • Top 9% globally <br>
🏅 **TCS CodeVita 2024**: All India Rank 1905  <br>
⭐ **HackerRank**: 5 Stars in Data Structures & Algorithms

</div>
</div>

<div style="max-width: 1000px; margin: 0 auto;">

## 🚀 Featured Projects

<center>
<table width="100%">
<tr>
<td width="50%">

### 🗣️ **VOICE: AI Language Assistant**
*AI-Powered Language Tutor | Mar 2025 - Current*

**🎯 Highlights:**
- 100% voice-based interaction using Whisper ASR
- Fine-tuned 110M-parameter LLMs for grammar correction
- <3 seconds average response latency
- 12% WER improvement with LanguageTool integration

**🛠️ Tech:** Next.js, LangChain, Flask, MongoDB, Groq LLMs

[![GitHub](https://img.shields.io/badge/View_Code-000000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/chinmaypandey62/VOICE-Your-Language-Assistant)

</td>
<td width="50%">

### 🎵 **Chord: Not Just a Chat App**
*Social Platform with Synchronized Experiences | Apr-May 2025*

**🎯 Highlights:**
- Real-time chat with Socket.IO
- WebRTC video calling
- Synchronized video playback across users
- Seamless state management

**🛠️ Tech:** Next.js, Express.js, MongoDB, Socket.IO, WebRTC

[![Live Demo](https://img.shields.io/badge/Live_Demo-FF5722?style=for-the-badge&logo=rocket&logoColor=white)](https://chord-app.onrender.com)
[![GitHub](https://img.shields.io/badge/View_Code-000000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/chinmaypandey62/chord)

</td>
</tr>
<tr>
<td width="50%">

### 💡 **IdeaIncubator**
*Collaborative Idea Sharing Platform | Jun-Jul 2024*

**🎯 Highlights:**
- Secure JWT-based authentication
- Post interactions (likes, comments)
- RESTful API architecture
- Full-stack deployment on Render

**🛠️ Tech:** Next.js, Node.js, MongoDB, JWT

[![Live Demo](https://img.shields.io/badge/Live_Demo-FF5722?style=for-the-badge&logo=rocket&logoColor=white)](https://ideaincubator.onrender.com)
[![GitHub](https://img.shields.io/badge/View_Code-000000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/chinmaypandey62/IdeaIncubator)

</td>
<td width="50%">

### 🤖 **Machine Learning Projects**
*Comprehensive ML Pipeline Development | Jan 2025 - Current*

**🎯 Highlights:**
- 80-90% accuracy on classification/regression tasks
- Advanced preprocessing (PCA, outlier removal)
- Multiple algorithms (XGBoost, RandomForest, KNN)
- Complete data science pipeline

**🛠️ Tech:** Python, Scikit-learn, TensorFlow, Pandas, NumPy

[![GitHub](https://img.shields.io/badge/View_Code-000000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/chinmaypandey62/ML-Projects)

</td>
</tr>
</table>
</center>
</div>

<br>

<div style="max-width: 1000px; margin: 0 auto;">

## 💼 Professional Journey

<center>
<table width="100%">
<tr>
<td width="60%">

### **Experience**

**🏢 Full Stack Web Developer Intern**  
*DecodeDaddy (SJSV Ventures Pvt Ltd) | Jun 2024 - Aug 2024*
- Built interactive landing page elements, improving user engagement
- Developed 10+ backend APIs using MongoDB for seamless data flow
- Collaborated with senior developers to implement responsive web solutions

### **Education**

**🏛️ Bachelor of Technology - Computer Science**  
*Pranveer Singh Institute of Technology, Kanpur | Expected 2026*
- Current GPA: 8.52/10.0 (Till 5th Semester)
- **Coursework:** Operating Systems, Data Structures, DBMS, OOP, System Design

</td>
<td width="40%">

### **Certifications**
- **React.js** - Infosys Springboard (2025)
- **JavaScript** - Infosys Springboard (2025)  
- **Machine Learning with Python** - Coursera (2025)
- **Python for Data Science** - IBM (2025)

### **Focus Areas**
- AI/ML Engineering
- Full Stack Development
- System Design
- Data Science & Analytics

</td>
</tr>
</table>
</center>
</div>

<br>

<div style="max-width: 1000px; margin: 0 auto;">

## 🤝 Let's Connect & Collaborate

<div align="center">

```md
🔥 Open to opportunities in Full Stack Development, AI/ML Engineering, and exciting collaborations!
```


<br>

[![Portfolio](https://img.shields.io/badge/🌟_Portfolio-FF5722?style=for-the-badge&logoColor=white)](http://chinmaypandey62.github.io/portfolio)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/chinmaypandey62)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/chinmaypandey62)
[![LeetCode](https://img.shields.io/badge/LeetCode-FFA116?style=for-the-badge&logo=leetcode&logoColor=black)](https://leetcode.com/u/chinmaypandey62)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:2201641520062@psit.ac.in)

<br>

> *"Code is poetry written in logic"*
> 
> Let's discuss web development projects, AI/ML innovations, competitive programming, or just tech in general!

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=18&duration=3000&pause=1000&color=6B7280&center=true&vCenter=true&width=600&lines=Thanks+for+visiting+my+profile!+%F0%9F%91%8B;Let's+build+something+amazing+together!+%F0%9F%9A%80;Happy+Coding!+%E2%9C%A8" alt="Footer Typing SVG" />

<br>

**© 2024 Chinmay Pandey | Made with ❤️ and lots of ☕**

</div>
</div>
